...
Public Class frmKismiFatura
    ...
    Private Sub btnOlustur_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOlustur.Click
        ...
        Dim sMalzemeler As String = ""
        Dim sSevkNolarTirnakli As String = ""
        Dim sMalzemelerTirnakli As String = ""
        ...
        Try
            'Dim seciliSevkNoItem As String = ""
            'Dim Malzemeler As String = ""
            'Dim count As Integer = 0

            Dim listSeciliSevkNoMalzeme As New List(Of String)

            'Dim listSevkNo As New List(Of String)

            listSeciliSevkNoMalzeme.Clear()
            ...
            For Each row In checkedRows
                ...
                'If seciliSevkNoItem = "" Then
                '    seciliSevkNoItem = row.Cells("     'Dim listSevkNo As New List(Of String)").Text & "|" & row.Cells("Item").Text
                'Else
                '    seciliSevkNoItem = seciliSevkNoItem & "," & row.Cells("SevkNo").Text & "|" & row.Cells("Item").Text
                'End If

                'If Not listSevkNo.Contains(row.Cells("SevkNo").Text) Then
                '    listSevkNo.Add(row.Cells("SevkNo").Text)
                'End If

                listSeciliSevkNoMalzeme.Add(row.Cells("SevkNo").Text & "|" & row.Cells("Item").Text)
            ...
            For Each row In checkedRows
                ...
                For Each s As String In listSeciliSevkNoMalzeme

                    If s.Contains(row.Cells("SevkNo").Text & "|" & row.Cells("Item").Text) Then
                        count = count + 1
                    End If
                ...
                sQuery = " Select * from TrM_FaturaMst_SL9 where SevkNo = " & row.Cells("SevkNo").Text & " and Item = " & sTirnakEkle(row.Cells("Item").Text)
                dtTemp = db.RunSql(sQuery)

                If dtTemp.Rows.Count > 0 Then

                    If dtTemp.Rows.Count <> count Then
                        MessageBox.Show(row.Cells("SevkNo").Text & " nolu sevkin " & row.Cells("Item").Text & " nolu malzemelerinin tüm kalemlerini seçiniz.")
                        Exit Sub
                    End If
            ...
            For Each row In checkedRows
                ...
                If Not sMalzemeler.Contains(row.Cells("Item").Text) Then
                    sMalzemeler = IIf(sMalzemeler = "", "", sMalzemeler & ",") & row.Cells("Item").Text
                End If
                ...
                If Not sMalzemelerTirnakli.Contains(row.Cells("Item").Text) Then
                    sMalzemelerTirnakli = IIf(sMalzemelerTirnakli = "", "'", sMalzemelerTirnakli & ",'") & row.Cells("Item").Text & "'"
                End If
            ...
            RaporCagir("SLFatura_Kismi.rpt", , , "fatura", False, False, , , paramFields)
            ...
            For Each row In checkedRows
                ...
                sQuery = " Update co_ship" & _
                            ...
                            " And shipment_id=" & sTirnakEkle(row.Cells("SEvkNo").Text) & _
                            " And ship_date =" & sTirnakEkle(CDate(row.Cells("ship_date").Text).ToString("yyyy-MM-dd HH:mm:ss")) & _
                            " And date_seq =" & sTirnakEkle(row.Cells("date_seq").Text)
            ...
            sQuery = "Update ShpPack" & _
                    ...
                    " Where SHPNO In (" & sSevkNolar & ")" & _
                    " And ITNBR In (" & sMalzemelerTirnakli & ")"
            ...
            sQuery = "Select cust_num, cust_seq,  co_num, co_line, co_release, qty_shipped,ROUND(dtyPrice * qty_shipped,2) As Tutar, ship_date, date_seq , SevkNo," & _
                        ...
                        " Where SEVKNO IN (" & sSevkNolar & ") And Item In (" & sMalzemelerTirnakli & ")"
    ...
    Private Sub GridEX1_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridEX1.CurrentCellChanged, GridEX1.Click, GridEX1.DoubleClick
        ...
        GridSec2(GridEX1, "SevkNo", "Item", sender, e)
