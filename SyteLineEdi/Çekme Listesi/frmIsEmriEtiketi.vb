Public Class frmIsEmriEtiketi
    ...
    Private Sub btnTemizle_Click(ByVal sender As System.Object, _
        ...
        Try
            ...
            EdtIsemriKodu.Text = String.Empty
            EdtIsemriTanimi.Text = String.Empty
        ...
        Try
            ...
            Else
                ...
                If MsgBox("Yazdırma işlemi yapılsın mı ?", MsgBoxStyle.YesNo, "Onay") = MsgBoxResult.Yes Then
                    ...
                    Dim ISEMRIKODU As String = String.Empty
                    ...
                    For Each row As Janus.Windows.GridEX.GridEXRow In GridEXBarkod.GetCheckedRows
                        ...
                        If row.Cells.Row.RowType = Janus.Windows.GridEX.RowType.Record Then
                            ...
                            ISEMRIKODU = row.Cells("Isemri_Kodu").Text
                    ...
                    AccessLogOnDatabase("Isemri_Etiketi.rpt", dbAccess.RunSql(sStr))
    ...
    Private Sub EdtIsemriKodu_Click(ByVal sender As System.Object, _
        ByVal e As System.EventArgs) Handles EdtIsemriKodu.ButtonClick
        Try

            sQuery = " SELECT job [İşemri Kodu], description [İşemri Tanımı] FROM job " & _
                     " WHERE type='J' " & _
                     " ORDER BY job "

            FindFormCagir(sQuery, "İşemri Kodu", "İşemri Tanımı", EdtIsemriKodu.Text, EdtIsemriTanimi.Text)

            If EdtIsemriTanimi.Text <> String.Empty Then
                'EdtIrsaliyeNo.Text = String.Empty
                'EdtIrsaliyeNo.Focus()
            End If
    ...
    Private Sub EdtIsemriKodu_KeyDown(ByVal sender As Object, _
        ByVal e As System.Windows.Forms.KeyEventArgs) Handles EdtIsemriKodu.KeyDown
        Try
            ...
            If e.KeyCode = Keys.Enter Then
                ...
                'If EdtIsemriTanimi.Text = String.Empty Then Throw New Exception("Lütfen ilk önce satıcı kodu seçiniz !")

                sQuery = " SELECT job [İşemri Kodu], description [İşemri Tanımı] FROM job " & _
                           " WHERE type='J' " & _
                           "   and job=" & sTirnakEkle(EdtIsemriKodu.Text)
                dt = db.RunSql(sQuery)

                If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                    EdtIsemriTanimi.Text = dt.Rows(0)("İşemri Tanımı").ToString
                    'EdtIrsaliyeNo.Text = String.Empty
                    'EdtIrsaliyeNo.Focus()
                End If
    ...
    Private Sub EdtIsemriKodu_KeyPress(ByVal sender As System.Object, _
        ByVal e As System.EventArgs) Handles EdtIsemriKodu.KeyPress
        ', _
        'EdtIrsaliyeNo.KeyPress
        Try

            Dim CtrlTmp As Control = CType(sender, Janus.Windows.GridEX.EditControls.EditBox)

            Select Case CtrlTmp.Name
                Case EdtIsemriKodu.Name
    ...
    Private Sub EdtIsemriKodu_TextChanged(ByVal sender As Object, _
        ByVal e As System.EventArgs) Handles EdtIsemriKodu.TextChanged
        Try

            If EdtIsemriKodu.Text = String.Empty Then
    ...
    Private Sub frmIsEmriEtiketi_KeyDown(ByVal sender As Object, _
        ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try

            If e.KeyCode = Keys.F4 Then
                If Me.ActiveControl Is EdtIsemriKodu Then
                    EdtIsemriKodu_Click(sender, e)
    ...
    Private Sub frmIsEmriEtiketi_Load(ByVal sender As System.Object, _
    ...
    Private Sub Listele()
        ...
        Try
            ...
            sQuery = " SELECT job AS	  [Isemri_Kodu], " & _
                     "        item AS      [Malzeme_Kodu], " & _
                     "        description   AS  [Malzeme_Tanimi],	" & _
                     "        isnull(qty_released,0)  AS  [Miktar], " & _
                     "        job           AS      [Lot_No], " & _
                     "        job_date AS    [Tarih], " & _
                     "       '' as [Etiket_Miktari], " & _
                     "       '' as [Kopya_Sayisi] " & _
            ...
            If EdtIsemriKodu.Text <> "" Then

                sQuery &= " And job=" & sTirnakEkle(EdtIsemriKodu.Text)
            ...
            'sQuery &= " And max(m.trans_date)>=" & sTirnakEkle(dtTarihBaslar.Value.ToString("yyyy-MM-dd"))

            'sQuery &= " And max(m.trans_date)<=" & sTirnakEkle(dtTarihBiter.Value.ToString("yyyy-MM-dd"))
    ...
    'Private Sub EdtIrsaliyeNo_KeyDown(ByVal sender As Object, _
    '                                  ByVal e As System.Windows.Forms.KeyEventArgs) _
    '                                  Handles EdtIrsaliyeNo.KeyDown
    '    Try
    '        Exit Sub
    '        If e.KeyCode = Keys.Enter Then
    '            If EdtIsemriTanimi.Text = String.Empty Then Throw New Exception("Lütfen ilk önce satıcı kodu seçiniz !")
    '            sQuery = " SELECT top 1 distinct m.document_num AS [IrsaliyeNo] " & _
    '               " FROM dbo.matltran m " & _
    '               " LEFT JOIN po p ON m.ref_num=p.po_num " & _
    '               " WHERE m.trans_type='R' AND m.ref_type='P' " & _
    '               " and p.vend_num=" & sTirnakEkle(EdtIsemriKodu.Text.PadLeft(7, String.Empty).ToString) & _
    '               " and m.document_num=" & sTirnakEkle(EdtIrsaliyeNo.Text) & _
    '               " GROUP BY " & _
    '               " m.document_num , p.vend_num ,  " & _
    '               " m.item, m.lot " & _
    '               " HAVING(SUM(m.qty) > 0)"
    '            'sQuery = " select document_num as [İrsaliye No] " & _
    '            '         " from matltran " & _
    '            '         " where vend_num=" & sTirnakEkle(EdtIsemriKodu.Text.PadLeft(7, String.Empty).ToString) & _
    '            '         " and document_num=" & sTirnakEkle(EdtIrsaliyeNo.Text)
    '            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
    '                EdtIrsaliyeNo.Text = dt.Rows(0)(0).ToString
    '            End If
    '        End If
    '    Catch ex As Exception
    '        MsgBox(ex.Message.ToString, MsgBoxStyle.Exclamation, "Hata")
