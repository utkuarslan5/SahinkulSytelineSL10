...
Public Class frmKonsinyeSatis
    ...
    Private Sub DosyaListele(ByVal sDizin As String)
        ...
        table.Columns.Add(New DataColumn("contno", Type.GetType("System.String")))
        ...
        Try
            ...
            Dim kontratNo As String = ""
            Dim rffType As String = ""
            'sVer = ""
            'sRel = ""
            'sMRef = ""
            'sMCRef = ""
            ...
            Do While Not strLine Is Nothing
                ...
                sTip = Copy(strLine, 1 - 1, 5).TrimEnd()
                ...
                ElseIf sTip = "BGM 3" Then

                    docNo = LTrim(Copy(strLine, 21, 40)).TrimEnd()

                    sQuery = "select * from KONSWRK WHERE DocNo = '" & docNo & "'"
                    dt = db.RunSql(sQuery)
                ...
                ElseIf sTip = "DTM 1" Then

                    If Copy(strLine, 5 - 1, 3).TrimEnd() = "137" Then

                        docDate = Copy(strLine, 8 - 1, 12).TrimEnd()

                    End If

                ElseIf sTip = "NAD C" Then
                ...
                ElseIf sTip = "QTY02" Then

                    If Copy(strLine, 5 - 1, 3).TrimEnd() = "12" Or Copy(strLine, 7 - 1, 3).TrimEnd() = "12" Then

                        qty = CDec(Copy(strLine, 10 - 1, 15).TrimEnd())

                    End If

                ElseIf sTip = "RFF05" Then


                    kontratNo = Copy(strLine, 10 - 1, 30).TrimEnd()


                    sQuery = "Select Top 1 BZMITM from kontrtpf " &
                        " where cust = " & sTirnakEkle(customer) & " And cust_seq =  " & cust_seq & " And ITEM =  " & sTirnakEkle(custItem) & " And KAPI = " & sTirnakEkle(gate) & " AND KNTRT=" & sTirnakEkle(kontratNo)
                    dt = db.RunSql(sQuery)
                    ...
                    table.Rows.Add(New Object() {msgNo, docNo, docDate, plant, gate, customer, cust_seq, custItem, kontratNo, qty, item, qty_on_hand, stat, ""})
    ...
    Private Sub btnMesajAl_Click(sender As System.Object, e As System.EventArgs) Handles btnMesajAl.Click
        Try
            ...
            For Each row As Janus.Windows.GridEX.GridEXRow In GridEX1.GetRows
                ...
                If CDec(row.Cells("Qty").Text) > CDec(row.Cells("qty_on_hand").Text) Then
            ...
            For Each row As Janus.Windows.GridEX.GridEXRow In GridEX1.GetRows
                ...
                If row.Cells("Customer").Text = "" Then
                    MessageBox.Show("Müşteri bilgisi boş olamaz.")
                    Exit Sub
                End If
            ...
            For Each row As GridEXRow In GridEX1.GetRows
                ...
                sQuery = "exec TR_KS_COsp " & sTirnakEkle(row.Cells("Customer").Text) & "," & row.Cells("cust_seq").Text & "," & sTirnakEkle(row.Cells("DocNo").Text)
                db.RunSql(sQuery)

                sQuery = "exec TR_COITEM_INSERT_CURSOR " & sTirnakEkle(row.Cells("DocNo").Text)
                db.RunSql(sQuery)

                Dim siparisNo As String = ""
                sQuery = "SELECT TOP 1 co_num from co where cust_po = " & sTirnakEkle(row.Cells("DocNo").Text)
                dt = db.RunSql(sQuery)

                If dt.Rows.Count > 0 Then
                    siparisNo = dt.Rows(0)("co_num").ToString
                End If

                'sQuery = "exec TR_Sevkiyat_KONS " & sTirnakEkle(siparisNo) & "," & sTirnakEkle(Date.Now.ToString("yyyy-MM-dd"))
                sQuery = "exec TR_Sevkiyat_KONS " & sTirnakEkle(siparisNo) & "," & sTirnakEkle(row.Cells("DocDate").Text.Substring(0, 8))
                db.RunSql(sQuery)

                MessageBox.Show("Sevkiyat Tamamlandı.")
